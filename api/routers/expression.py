...
from api.models.schemas import ExpressionCreate, ExpressionIn, ExpressionOut

router = APIRouter(dependencies=[Depends(JWTBearer())], tags=["expressions"])
...
def get_expressions(db: Session = Depends(get_db)):
    try:
        ...
        return {
            "expressions": [
                {"id": r[0], "expression": r[1], "result": r[2]} for r in result
            ]
        }
    except Exception:
...
def delete_expression(expression_id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Error deleting expression: {info}"
        )
...
def evaluate_expression(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid expression or values: {info}"
        )

    return {
        "expression": expression,
        "result": result,
        "values": {"x": x, "y": y, "z": z, "j": j, "k": k},
    }


@router.post("/expressions", response_model=ExpressionCreate)
def create_expression(expression_in: ExpressionIn, db: Session = Depends(get_db)):
    expression = expression_in.expression
    result = 0.0
    try:
        result = db.execute(
            text(
                "INSERT INTO expression (expression, result) VALUES (:expression, :result) RETURNING id"
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Error inserting expression: {info}"
        )
